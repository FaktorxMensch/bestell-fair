<template>
  <v-card>
    <v-card-title>Restaurant bearbeiten</v-card-title>
    <v-card-text>
      <!-- have as grid of three two or one, depending on screen size -->
      <v-form>
        <v-text-field label="Name des Restaurants" v-model="restaurant.name"/>
        <v-text-field label="Beschreibungs und Willkommens Text" v-model="restaurant.description"/>
        <input type="file" @change="uploadImage('icon', $event)">
        <input type="file" @change="uploadImage('feature', $event)">
        ...
        <div class="grid grid-cols-3 md:grid-cols-2 gap-x-4">
          ...
          <v-select label="Felder, die bei einer Bestellung ausgefüllt werden können" v-model="restaurant.order_fields"
                    multiple
                    chips
                    :items="['Name', 'Telefon', 'E-Mail', 'Anmerkungen', 'Tischnummer', 'Lieferadresse' ]"></v-select>
          <v-select label="Tags" v-model="restaurant.tags" multiple
                    chips
                    :items="['Vegan', 'Vegetarisch', 'Glutenfrei', 'Laktosefrei', 'Bio', 'Fairtrade', 'Regional', 'Saisonal']"></v-select>
